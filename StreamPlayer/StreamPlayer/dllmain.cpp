...
STREAMPLAYER_API int32_t __stdcall StartPlay(const char* url,
	uint32_t connectionTimeoutInMilliseconds, FFmpeg::Facade::RtspTransport transport,
	FFmpeg::Facade::RtspFlags flags, FFmpeg::Facade::FrameDelay framedelay)
{
    try
    {
        player.StartPlay(url, connectionTimeoutInMilliseconds, transport, flags, framedelay);
    }
    catch (std::runtime_error &)
    {
        return 1;
