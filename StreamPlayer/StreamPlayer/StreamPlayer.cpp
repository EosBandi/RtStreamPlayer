...
void StreamPlayer::StartPlay(string const& streamUrl,
    uint32_t connectionTimeoutInMilliseconds, RtspTransport transport, RtspFlags flags, FrameDelay framedelay)
{

	workerThread_ = boost::thread(&StreamPlayer::Play, this,
        streamUrl, connectionTimeoutInMilliseconds, transport, flags, framedelay);
}
    ...
    int32_t connectionTimeoutInMilliseconds, RtspTransport transport, RtspFlags flags, FrameDelay framedelay)
{
	...
	if (framedelay == FrameDelay::Zero)
	{
		printf("Framedelay disabled\n");

	}
	else
	{
		printf("FrameDelay Enabled\n");
	}
	...
	{
        {
            ...
            stream_ = make_unique<Stream>(streamUrl,
				connectionTimeoutInMilliseconds, transport, flags, framedelay);
        }
		...
		for (;;)
		{
			...
			if (framedelay == FrameDelay::Calculated)
			{
